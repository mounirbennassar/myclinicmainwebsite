...
/**
 * Pediatric booking form. Leads are tagged with the `pediatric` vertical so the
 * dashboard isolates them from general medical and dental leads (via the
 * Pediatric toggle). Requires `pediatric` in the appointments vertical CHECK
 * constraint — see scripts/pediatric-vertical-migration.sql. The chosen child
 * age band is appended to the name so the call centre sees it; campaign
 * attribution still flows through UTMs.
 */
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/appointments", {
        ...
        body: JSON.stringify({ city, name: leadName, phone, vertical: "pediatric", utm, referrer }),
      });
      if (res.ok) {
        trackFormSubmit();
        setSuccess(true);
      ...
      <div className="relative max-w-3xl mx-auto px-4 md:px-8">
        <div className="text-center mb-20 md:mb-24">
          ...
          <h2 className={`mt-4 font-headline text-3xl md:text-4xl font-extrabold text-slate-900 ${isRtl ? "" : "tracking-tight"}`}>
            {isRtl ? "اطمئن على صحة طفلك واحجز موعدك بكل سهولة" : "Peace of mind is one click away"}
          </h2>
          <p className="mt-3 text-slate-500 font-medium [text-wrap:pretty]">
            {isRtl
              ? "سيتواصل معك فريقنا لتأكيد الموعد ومساعدتك في اختيار الخدمة والطبيب المناسب."
              : "Book your child's appointment in seconds. Our team will reach out to confirm and help you choose the right service and doctor."}
          </p>
        </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className={labelCls}>
                      ...
                      {isRtl ? "رقم التواصل" : "Contact number"}
                    </label>
                ...
                >
                  {submitting ? (
                    ...
                    <>
                      ...
                      {isRtl ? "احجز موعد طفلي" : "Book my child's appointment"}
                    </>
                  )}
                </button>
